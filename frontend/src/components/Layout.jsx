          ...
          <div className="absolute top-14 left-0 right-0 bg-white dark:bg-slate-900 border-b border-gray-100 dark:border-slate-800 shadow-xl z-50 animate-slide-down">
            <div className="max-w-7xl mx-auto px-4 py-5">
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-1 mb-5">
                <Link to="/" className="flex items-center gap-2 px-4 py-3 rounded-xl text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-amber-50 dark:hover:bg-slate-800 hover:text-amber-600 transition-colors">
                  🏠 Home
                </Link>
                {CATEGORIES.map(c => (
                  <Link key={c.id} to={`/category/${c.id}`}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 mb-6">
            ...
            {CATEGORIES.slice(0, 3).map(c => (
              <div key={c.id}>
                <p className="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide mb-2">{c.name}</p>
                <div className="space-y-1">
                  {/* just link to category */}
                  <Link to={`/category/${c.id}`} className="block text-xs text-gray-400 hover:text-amber-500 transition-colors">
                    Browse {c.name}
                  </Link>
                </div>
              </div>
